<script>
// import {set as setElem} from "lodash"
class CronUI {
  el = null
  ...
  render () {
    ...
    // Period
    el.insertAdjacentHTML("beforeend", "<span class='cron-period'><select>" + str_opt_period + "</select></span>")
    const periodEl = el.querySelector(".cron-period select")
  ...
  periodChanged () {
    ...
    const cronPeriodEl = this.el.querySelector(".cron-period select")
    const period = cronPeriodEl.options[cronPeriodEl.selectedIndex].value
    ...
    if (CronUI.displayMatrix.hasOwnProperty(period)) {
      const b = CronUI.displayMatrix[period]
  ...
  getCronString () {
    ...
    const getSelectValue = function (el) {
      return el.options[el.selectedIndex].value
    }

    const selectedPeriod = getSelectValue(this.el.querySelector(".cron-period select"))
  ...
  setCronString  (cronString) {
    const blocks = this.blocks
    const cronType = CronUI.getCronType(cronString)

    if (!cronType) { return false }

    const d = cronString.split(" ")
    const v = {
      mins: d[0],
      hour: d[1],
      dom: d[2],
    ...
    const targets = CronUI.displayMatrix[cronType]
  ...
  static getCronType  (cronString) {
    ...
    for (const type in CronUI.cronTypes) {
...
export default {
  ...
  watch: {
    cronText (newValue) {
      ...
      if (!cronUILoaded.setCronString(this.cronText)) {
        this.$emit("invalidCron")
      }
    }
  },
  mounted () {
    cronUILoaded = new CronUI(this.$el, {
      initial: this.cronText,
      changeEvent: (v) => {
        this.$emit("update:cronText", v)
      }
    })
  }
}
